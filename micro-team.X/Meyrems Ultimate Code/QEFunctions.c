...
void __attribute__((interrupt, auto_psv)) _QEI1Interrupt(void)
{
    ...
    if(POSCNT<32768){
        positionLEFT+=0xFFFF;   // over-run condition caused interrupt
    }else{
        positionLEFT-=0xFFFF;   // under-run condition caused interrupt
    } 
}
...
void __attribute__((interrupt, auto_psv)) _QEI2Interrupt(void)
{
    ...
    if(POSCNT<32768){
        positionRIGHT+=0xFFFF;  // over-run condition caused interrupt
    }else{
        positionRIGHT-=0xFFFF;  // under-run condition caused interrupt
    } 
}