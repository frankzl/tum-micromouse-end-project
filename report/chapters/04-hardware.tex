...
In order for the right width to be chosen, we need to consider the desirable value for the resistance. And in order to do that, again $\Omega$mhs Law has to be considered, referring to the current we expect to run through our trace.
...
The last concept to be introduced in this chapter is the "ground plane" (GND). The ground plane is a convenient way of connecting all the GND nodes (pins) of the components. In essence, it is a layer placed on one of the PCB layers, which connects everything on it. It can be seen in Fig. \ref{fig:gnd}.
...
%this might be understood as that only one GND plane is allowed.
Remember that the GND surface is applied to one of the layers, in our case, the top. What about the SMD components that are placed on the bottom layer, but still need tp have one or more of their pins connected to GND? The solution might not seem very elegant, but is functional: A trace is runs from the gnd pin of the component, on the bottom layer, and ends in a via point. Hence, it connects to the GND on the top layer. An example from our circuit can be seen in Fig. \ref{fig:via}.
